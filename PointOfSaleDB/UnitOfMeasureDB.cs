...
{
    public class UnitOfMeasureDB : ICustomRepo<CustomDBHelper, UnitOfMeasureDTO, string>
    {
        public UnitOfMeasureDB()
        {
            this.conn = new CustomDBHelper(CustomCurrentLogin.connString);
            this.tableName = "tblunitofmeasure";
        }
        ...
        public UnitOfMeasureDTO GetById(string idTrack)
        {
            return conn.GetData<UnitOfMeasureDTO>($"SELECT * FROM {tableName} WHERE IdTrack = {idTrack}");
        }
        ...
        {
            conn.FillCustomComboBox(cmb, "UnitOfMeasure", "IdTrack", $"SELECT * FROM {tableName}");
        }
        ...
        public bool SaveData(UnitOfMeasureDTO sDTO)
        {
        ...
        public bool UpdateData(UnitOfMeasureDTO sDTO)
        {
            if (sDTO.DataValidation())
            {
